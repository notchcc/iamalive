...
個人用旅行報平安：一鍵打卡、家人群組 LINE 通知、逾時警報、雙時區家人頁。規格見 [`docs/spec.md`](docs/spec.md)（v0.10）。
...
2. 出門後點主畫面的打卡頁圖示或按捷徑 A 打卡；或**直接在 LINE 私訊官方帳號傳送「位置」**，也算一次打卡；先傳照片再傳位置（15 分鐘內）會記成含照片的打卡。打卡頁選圖庫照片時可勾選「以拍攝時間為打卡時間」，補記稍早的位置。打卡不推群組；家人看家人頁，或在群組輸入「在哪」「行程」。
3. 長途飛行前按捷徑 C 或在群組輸入 `離線 16`。打錯卡：`/me` 打卡管理刪除，或私訊 `刪除最後一筆`。行程中要改打卡頻率：`/me` 行程管理，或輸入 `頻率 6`（期限立即重算）。
4. 期限若落在你當地的睡眠時段（預設 23:00–08:00，行程管理可改）會自動順延到起床後 1 小時，睡前忘了打卡不會半夜誤發警報。回報期限前 1 小時，官方帳號會私訊提醒你本人（要先加 @574stmif 為好友；每個期限一次）。
