...
- **狀態卡收合**：有打卡紀錄時，主區只留行程標題、狀態大字（「最後回報：N 小時前」等）與警示類副標（超時 / 離線 / 飛行中 / 已結束）。最後打卡的地點、時間、備註與下次期限縮進右上角「📍 地點」鈕（地點取 `place` 逗號前的城市名，長字省略）的面板，點一下展開 / 收合，預設收合。面板為兩列「標籤 / 內容」：「最後打卡」列放地點、雙時區時間、備註；「下次期限」列（超時後改「預定回報」）放雙時區時間，順延原因以小標籤（睡眠時段順延 / 航段順延）附在後面，不用圖示。尚未回報時沒有此鈕，首次期限直接顯示在主區。
