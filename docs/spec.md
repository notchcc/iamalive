...
| v0.10 | **睡眠時段**：期限落在旅人當地睡眠時段內自動順延到結束後 1 小時，提醒與警報都依有效期限；家人頁 / 打卡頁 / LINE 顯示順延原因；view 帶 `effectiveDeadlineAt` 與 `deadlineShift`；家人頁狀態卡與打卡頁同款「下次期限 …（睡眠時段順延）」，不再顯示「下一段」航班（僅飛行中顯示航班）；**24 小時警報預報**時間軸（管理頁、打卡頁） |
...
- **私訊官方帳號**或在**已綁定的家人群組**按「＋ → 位置資訊 → 傳送」，約 3 次點擊、5–8 秒。
- webhook 收到後視為打卡（§5.3）：私訊以傳訊者 userId 對應其行程；群組以 `groups/{gid}.ownerUid` 對應且只接受擁有者。只 reply 確認，不另外 push。
- 私訊亦支援 `離線 N`、`結束`、`備註 xxx`、`在哪`、`行程`；其他文字回覆使用說明；`follow`（加好友）回覆歡迎與說明。
- **圖片訊息**（私訊或群組內擁有者傳送）：LINE 轉送的圖片通常已移除 EXIF，因此採「照片與位置配對」：(1) 圖片帶 GPS → 直接照片打卡；(2) 15 分鐘內剛打過卡且無照片 → 附到那筆；(3) 否則存入 `pendingPhotos/{uid}`（15 分鐘），之後的位置訊息合成一筆 `source = photo` 的打卡。過期未配對的照片由 `checkOverdue` 順帶清除。
...
- 頁面順序：狀態卡（最後回報多久前、地點、下次期限、離線）→ 打卡卡片（上半是位置地圖：藍點為目前位置、其餘為最近 5 次打卡；下半是三個大按鈕「定位打卡 / 拍照打卡 / 選擇照片」，再下方「進階選項」預設折疊，內含備註與下次回報，有填內容時摘要顯示在標題旁）→ 24 小時預報 → 最近 5 次打卡 → 本頁連結。從圖庫選的照片若拍攝時間在過去 7 天內且非剛拍，會出現「以拍攝時間為打卡時間」勾選（預設不勾）。拍照走 `capture=environment`，沒有 GPS 時自動改用目前定位。頁面可見（切回前景）時自動重新整理，30 秒更新「多久前」。
...
### 8.1.2 版面精簡、照片幻燈片與時間軸分頁

- 「本頁連結 ▾」放在頁面最下方（地圖之後、頁尾之前），預設折疊，點開才顯示網址、複製 / 開啟。
- 狀態卡與打卡頁同款：「下次期限 台北 …（當地 …）（睡眠時段順延 / 航段順延）」；航班只在飛行窗內顯示（標題「✈️ 飛行中 XX」），不顯示「下一段」。
- **狀態卡收合**：有打卡紀錄時，主區只留行程標題、狀態大字（「最後回報：N 小時前」等）與警示類副標（超時 / 離線 / 飛行中 / 已結束）。最後打卡的地點、時間、備註與下次期限縮進右上角「📍 地點」鈕（地點取 `place` 逗號前的城市名，長字省略）的面板，點一下展開 / 收合，預設收合。面板為兩列「標籤 / 內容」：「最後打卡」列第一行為雙時區時間，第二行以淡色小字放地點，再來是備註；「下次期限」列（超時後改「預定回報」）放雙時區時間，順延原因以小標籤（睡眠時段順延 / 航段順延）附在後面，不用圖示。尚未回報時沒有此鈕，首次期限直接顯示在主區。
- 狀態卡下方為**照片幻燈片**（原本的獨立 `/g/` 明信片頁已移除，隨機輪播與地圖跟隨併入此處）：
  - 資料：`recent` 內有照片的先進牌堆，再以 `GET /api/w/{token}/checkins?photos=1&limit=50&before=…` 在背景續抓到整趟載完（上限 200 張；該模式只回有照片的並附掃描游標與 `exhausted`）。快播完一輪時再補下一頁，新載入的照片混入尚未播到的部分；`recent` 進來新照片時排在下一張。沒有照片就隱藏整區。
  - 版面：滿版照片、圓角 14px、4:3（最高 56vh）、深色底；底部漸層字幕為地點（`place`，查不到退回時區城市名）、打卡當地日期時間與備註。不顯示計數，卡片上不放下載鈕。
  - 切換：隨機洗牌不重複，一輪播完再洗；每 7 秒自動換下一張，水平滑入滑出並預載下一張；分頁隱藏或全螢幕檢視中不換。左右各一顆半透明圓形箭頭（右側入場時輕推 3 次）：點左 = 上一張（第一張時淡化停用）、點右 = 下一張；箭頭 5 秒沒操作就淡出，手指碰到卡片 / 滑鼠移入再出現，淡出後左右滑動（水平位移 ≥ 40px）與 ← → 鍵仍有效。
  - 點照片進入全螢幕檢視（與照片回顧頁同一元件）：帶入已載入的全部照片（新到舊）從當前那張開始，可左右滑動 / 箭頭 / 方向鍵看其他張；右上角 X 關閉與下載鈕（檔名 `英文地名_日期.jpg`，下載的是伺服器上 1600px 的版本），點背景或 Esc 關閉，檢視中鎖住頁面捲動並暫停輪播。關閉時若看的不是原本那張，幻燈片切到那一張並視同手動換張（地圖跟著飛）。
- **地圖**預設框住時間軸已載入的打卡點與軌跡（資料未變動時不重畫）。幻燈片換張時（自動輪播與手動都算）飛到該張照片的拍攝地並以紅點加光暈標示，右上角出現「顯示全部打卡點」鈕，按下回到框住全部的視角並移除紅點。使用者剛拖曳 / 縮放地圖後 20 秒內，自動輪播不搶視角（手動換卡仍會飛）。第一張出現時不飛，維持預設視角。
...
- **時間軸改為底部抽屜**（頁面本體只剩本頁連結、雙時鐘、狀態卡、幻燈片、地圖，地圖畫**全部已同步的點**，不再只畫已載入的 10 筆）。抽屜三檔：
  - `peek`（預設）：只露出頂部的窺視條：最新一筆（或被點的那筆，標「已選」）的縮圖 / 圓點、台北時間、地點、多久前、備註，右側 ▴。點窺視條或往上拖 → `half`。
  - `half`（50dvh）/ `full`（92dvh）：標題「打卡紀錄 N 筆」與 ▾ 收合鈕，下方為可捲動的時間軸清單（lazy loading 以抽屜內捲動容器偵測，每次 10 筆）。背景變暗，點背景 / ▾ / Esc / 手機返回鍵回到 `peek`（展開時推一筆歷史，返回鍵先收抽屜再離開頁面）。
  - 只有頂部（拖曳條、窺視條、標題列）可拖，放開後吸附到最近一檔；清單區維持正常捲動。桌機同一份，寬度上限 720px 置中。
- **點時間軸卡片**（縮圖、座標連結、刪除鈕除外）**或地圖上的打卡點**（popup 照常彈出）：抽屜縮回 `peek` 並把該筆放到窺視條；該筆有照片就讓上方幻燈片切到那張（視同手動換張，地圖飛到拍攝地標紅點）並捲到幻燈片；沒有照片就只在地圖標出該筆位置並捲到地圖。被點的卡片以品牌色外框標示。
- 時間軸預設只渲染 10 筆；哨兵進入抽屜可視區就再顯示 10 筆。`recent`（最多 100 筆、即時）用完後改呼叫 `GET /api/w/{token}/checkins?before=…` 取更舊的紀錄，直到回傳不足 10 筆為止。
...
- 頁面：最上方「本頁連結 ▾」（折疊）、行程名稱與「網格 / 地圖」分段切換，**預設網格**（`#map` 記住地圖模式）。
  ...
  - 地圖檢視：上方是**家人頁同款的照片幻燈片**（隨機輪播每 7 秒、左右箭頭 5 秒淡出、點照片開全螢幕檢視可左右滑看全部；高度 min(38dvh, 4:3)），下方為地圖；幻燈片換張（含自動輪播）時地圖飛到該張拍攝地並以紅點加光暈標示，右上角出現「顯示全部照片」鈕可回到框住全部；使用者剛拖曳 / 縮放地圖後 20 秒內自動輪播不搶視角。地圖本身比照 iPhone 圖庫，切換時先把整趟載完；照片以 56px 圓角縮圖釘標示，同一畫面 64px 內的合併成一群（縮圖用最新一張，右上角數字為張數），拖曳 / 縮放後重新聚合；點群放大到該群範圍（最大 17 級）；只有一張或已放到最大時，該張顯示到上方幻燈片（群內其餘的接著播），要全螢幕再點幻燈片。首次進入框住全部照片。
  - 兩種模式點照片都進**全螢幕檢視**（與家人頁同一個元件）：右上角 X 與下載（檔名 `英文地名_日期.jpg`），點背景 / Esc 關閉，鎖住捲動；多張時可左右滑動（照片跟著手指移動，放開超過 48px 就滑入下一張，否則彈回）、點左右兩側的半透明圓形箭頭（與家人頁幻燈片同款，5 秒沒操作淡出、碰畫面再出現）或 ← → 切換，切換有滑入滑出動畫；底部左側為地點與當地時間，右側「n / N」。網格從整份清單的該張開始，地圖從該群的第一張開始。
