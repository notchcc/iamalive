...
- 版面：深色棉麻質感背景，置中一張 3:2 明信片（手機為 2:3 直式）：左（上）為白框照片，右（下）為訊息區，含郵票（行程名稱）、郵戳（當地日期縮寫 + 城市）、地點大字、當地完整日期與時間、備註（手寫體），沒備註顯示 “Wish you were here.”。
- 切換：隨機洗牌不重複，一輪播完再洗；每 7 秒自動換下一張，點一下 / 右滑 / → 鍵下一張，左滑 / ← 鍵回上一張；卡片以飛入飛出加微幅旋轉切換，並預載下一張；有暫停鍵；分頁隱藏時不換。
