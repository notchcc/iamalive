...
- 頁面：最上方「本頁連結 ▾」（折疊）、行程名稱與「網格 / 地圖」分段切換，**預設網格**（`#map` 記住地圖模式）。
  ...
  - 地圖檢視：上方是**家人頁同款的照片幻燈片**（隨機輪播每 7 秒、左右箭頭 5 秒淡出、點照片開全螢幕檢視可左右滑看全部；高度 min(38dvh, 4:3)），下方為地圖；幻燈片換張（含自動輪播）時地圖飛到該張拍攝地並以紅點加光暈標示，右上角出現「顯示全部照片」鈕可回到框住全部；使用者剛拖曳 / 縮放地圖後 20 秒內自動輪播不搶視角。地圖本身比照 iPhone 圖庫，切換時先把整趟載完；照片以 56px 圓角縮圖釘標示，同一畫面 64px 內的合併成一群（縮圖用最新一張，右上角數字為張數），拖曳 / 縮放後重新聚合；點群放大到該群範圍（最大 17 級），已到最大或只有一張則直接開檢視。首次進入框住全部照片。
