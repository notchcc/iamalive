...
- 版面：深色棉麻質感背景，上方置中一張 4:3 的明信片（白框照片，隨機微傾斜）；照片左下角疊上英文地名大寫與「國名 · 當地日期縮寫」，底部柔和漸層，全部同一字體 Noto Serif TC。地名規則：`placeEn` 的城市 → 沒有城市就用國名 → 都沒有只顯示日期（`placeEn` 於打卡時以 Nominatim `accept-language=en` 另存，舊照片由 `photos=1` 分頁每次補 4 筆寫回）。右上角有下載鈕下載目前照片，下方是一張**不傾斜、可互動**的地圖（高 34dvh，可拖曳縮放），換卡時飛到該張照片的拍攝地點並以紅點標示。
