...
- 版面：深色棉麻質感背景，上方置中一張 4:3 的明信片（白框照片，隨機微傾斜；郵戳蓋在照片左上角：矩形雙框章，第一行**英文城市名**大寫（`placeEn`，打卡時以 Nominatim `accept-language=en` 另存；舊照片由 `photos=1` 分頁每次補 4 筆寫回），第二行英文國名 + 當地日期縮寫，右側消印波紋；不放其他文字。右上角有下載鈕下載目前照片），下方是一張**不傾斜、可互動**的地圖（高 34dvh，可拖曳縮放），換卡時飛到該張照片的拍攝地點並以紅點標示。
- 切換：隨機洗牌不重複，一輪播完再洗；每 7 秒自動換下一張，點一下 / 右滑 / → 鍵下一張，左滑 / ← 鍵回上一張；卡片以飛入飛出加微幅旋轉切換，並預載下一張；有暫停鍵；分頁隱藏時不換；頁面底部不放任何文字。
- 字體以 Google Fonts 載入 Caveat（手寫）與 Noto Serif TC，失敗時退回系統字體。
