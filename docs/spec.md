...
### 8.1.2 版面精簡、幻燈片與時間軸分頁

- 頁面最上方只有一行小字「本頁連結 ▾」，預設折疊，點開才顯示網址、複製 / 開啟。
- 狀態卡與打卡頁同款：「下次期限 台北 …（當地 …）（睡眠時段順延 / 航段順延）」；航班只在飛行窗內顯示（標題「✈️ 飛行中 XX」），不顯示「下一段」。
- 幻燈片每張右上角有下載鈕（原圖）。
- 狀態卡下方為**最近照片幻燈片**：先放 `recent` 內有照片的前 10 張，橫向 scroll-snap，每 5 秒自動輪播，使用者滑動後暫停 10 秒；滑到倒數第 2 張就再補 10 張（先用 `recent` 其餘，再以 `GET /api/w/{token}/checkins?photos=1&before=…` 續抓，該模式只回有照片的並附掃描游標與 `exhausted`）；右下計數「3 / 12+」，字幕為地點與打卡當地日期時間，點圖開原圖；沒有照片就隱藏。
- **航段**改為可折疊（`<details>`），預設收合，摘要列顯示段數與「飛行中 XX」或「下一段 XX 起飛時間」。
- 時間軸預設只渲染 10 筆；底部哨兵進入視窗（IntersectionObserver）就再顯示 10 筆。`recent`（最多 100 筆、即時）用完後改呼叫 `GET /api/w/{token}/checkins?before=…` 取更舊的紀錄，直到回傳不足 10 筆為止。地圖只畫時間軸已載入的點與軌跡，資料未變動時不重畫。

### 8.1.3 明信片展示頁 `/g/{readToken}`

- 與家人頁同一個 token，權限相同（照片走 `/api/p/{token}/{id}`）；家人頁幻燈片下方有「🖼 明信片模式」連結，每趟行程一個獨立網址。
- 資料：`GET /api/w/{token}` 取行程名稱，`GET /api/w/{token}/checkins?photos=1&limit=50` 分頁收集有照片的打卡（最多 200 張）。
- 版面：深色棉麻質感背景，上方置中一張 4:3 的明信片（白框照片，隨機微傾斜）；照片左下角疊上英文地名大寫與「國名 · 當地日期縮寫」，底部柔和漸層，全部同一字體 Noto Serif TC。地名規則：`placeEn` 的城市 → 沒有城市就用國名 → 都沒有只顯示日期（`placeEn` 於打卡時以 Nominatim `accept-language=en` 另存，舊資料由 `checkOverdue` 每 15 分鐘補最多 8 筆（間隔 1.1 秒），`photos=1` 分頁另外每次補 2 筆）。明信片左右兩側各有半透明圓形箭頭（右側入場時輕微推動 3 次暗示可滑動）：點左 = 上一張（第一張時淡出停用）、點右 = 下一張，左右滑動與方向鍵亦可。點照片進入全螢幕檢視（右上角 X 關閉與下載鈕；檢視中暫停輪播並隱藏地圖），頁首不放下載鈕，下方是一張**不傾斜、可互動**的地圖（高 34dvh，可拖曳縮放），換卡時飛到該張照片的拍攝地點並以紅點標示。
- 切換：隨機洗牌不重複，一輪播完再洗；每 7 秒自動換下一張，點一下 / 右滑 / → 鍵下一張，左滑 / ← 鍵回上一張；卡片以飛入飛出加微幅旋轉切換，並預載下一張；有暫停鍵；分頁隱藏時不換；頁面底部不放任何文字。
- 字體以 Google Fonts 載入 Noto Serif TC，失敗時退回系統襯線字體。
