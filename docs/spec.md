...
- 狀態卡下方為**明信片牌堆**（取代原本的幻燈片與獨立的 `/g/` 明信片頁，後者已移除）：
  ...
  - 版面：不另加底色，直接在頁面上放一張 4:3 明信片（白框照片、柔和陰影、隨機 ±2.5° 微傾斜，最高 56vh）；照片左下角疊英文地名大寫與「國名 · 當地日期縮寫」，底部柔和漸層，字體 Noto Serif TC（Google Fonts，失敗退回系統襯線體）。地名規則：`placeEn` 的城市 → 沒有城市就用國名 → 都沒有只顯示日期（`placeEn` 於打卡時以 Nominatim `accept-language=en` 另存，舊資料由 `checkOverdue` 每 15 分鐘補最多 8 筆，間隔 1.1 秒）。不顯示計數，卡片上不放下載鈕。
