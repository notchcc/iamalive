...
- **狀態卡收合**：有打卡紀錄時，主區只留行程標題、狀態大字（「最後回報：N 小時前」等）與警示類副標（超時 / 離線 / 飛行中 / 已結束）。最後打卡的地點、時間、備註與「下次期限 …」縮進右上角「📍 地點」鈕（地點取 `place` 逗號前的城市名，長字省略）的面板，點一下展開 / 收合，預設收合。尚未回報時沒有此鈕，首次期限直接顯示在主區。
