...
### 8.1.2 版面精簡、明信片牌堆與時間軸分頁

- 頁面最上方只有一行小字「本頁連結 ▾」，預設折疊，點開才顯示網址、複製 / 開啟。
- 狀態卡與打卡頁同款：「下次期限 台北 …（當地 …）（睡眠時段順延 / 航段順延）」；航班只在飛行窗內顯示（標題「✈️ 飛行中 XX」），不顯示「下一段」。
- 狀態卡下方為**明信片牌堆**（取代原本的幻燈片與獨立的 `/g/` 明信片頁，後者已移除）：
  - 資料：`recent` 內有照片的先進牌堆，再以 `GET /api/w/{token}/checkins?photos=1&limit=50&before=…` 在背景續抓到整趟載完（上限 200 張；該模式只回有照片的並附掃描游標與 `exhausted`，每次另補 2 筆 `placeEn`）。快播完一輪時再補下一頁，新載入的照片混入尚未播到的部分；`recent` 進來新照片時排在下一張。沒有照片就隱藏整區。
  - 版面：深色棉麻質感底、圓角 14px 的區塊，內放一張 4:3 明信片（白框照片、隨機 ±2.5° 微傾斜，最高 56vh）；照片左下角疊英文地名大寫與「國名 · 當地日期縮寫」，底部柔和漸層，字體 Noto Serif TC（Google Fonts，失敗退回系統襯線體）。地名規則：`placeEn` 的城市 → 沒有城市就用國名 → 都沒有只顯示日期（`placeEn` 於打卡時以 Nominatim `accept-language=en` 另存，舊資料由 `checkOverdue` 每 15 分鐘補最多 8 筆，間隔 1.1 秒）。不顯示計數，卡片上不放下載鈕。
  - 切換：隨機洗牌不重複，一輪播完再洗；每 7 秒自動換下一張，卡片以飛入飛出加微幅旋轉切換並預載下一張；分頁隱藏或全螢幕檢視中不換。左右各一顆半透明圓形箭頭（右側入場時輕推 3 次）：點左 = 上一張（第一張時淡化停用）、點右 = 下一張；箭頭 5 秒沒操作就淡出，手指碰到卡片 / 滑鼠移入再出現，淡出後左右滑動（水平位移 ≥ 40px）與 ← → 鍵仍有效。
  - 點照片進入全螢幕檢視：右上角 X 關閉與下載鈕（檔名 `地名_日期.jpg`，下載的是伺服器上 1600px 的版本），點背景或 Esc 關閉，檢視中鎖住頁面捲動並暫停輪播。
- **地圖**預設框住時間軸已載入的打卡點與軌跡（資料未變動時不重畫）。明信片換卡時（自動輪播與手動都算）飛到該張照片的拍攝地並以紅點加光暈標示，右上角出現「顯示全部打卡點」鈕，按下回到框住全部的視角並移除紅點。使用者剛拖曳 / 縮放地圖後 20 秒內，自動輪播不搶視角（手動換卡仍會飛）。第一張明信片出現時不飛，維持預設視角。
